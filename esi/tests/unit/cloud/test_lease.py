...
@mock.patch.object(ks_base.BaseIdentityPlugin,
                   'get_endpoint_data',
                   return_value=fakes.get_lease_endpoint())
class TestLease(base.TestCase):
    def setUp(self):
        super(TestLease, self).setUp()
        self.fake_offer = fakes.make_fake_offer("fake_offer_id",
                                                "fake_node_id",
                                                "fake_type")
        self.fake_lease = fakes.make_fake_lease("fake_lease_id",
                                                "fake_node_id",
                                                "fake_type",
                                                "fake_offer_id")
        self.fake_node = fakes.make_fake_node("fake_node_id",
                                              "fake_offer_id",
                                              "fake_lease_id")
        self.fake_node_1 = fakes.make_fake_node("fake_node_id_1",
                                                "fake_offer_id_1",
                                                "fake_lease_id_1")
        self.fake_event = fakes.make_fake_event("fake_event_id",
                                                "fake_type",
                                                "fake_time")
        self.fake_event_1 = fakes.make_fake_event("fake_event_id_1",
                                                  "fake_type",
                                                  "fake_time_1")
        self.cloud_config = self.config.get_one(cloud='_test_cloud_',
                                                validate=True)
    ...
    def test_list_offers(self, mock_ged):
        fake_offer_1 = fakes.make_fake_offer("fake_offer_id_1",
                                             "fake_node_id_1",
                                             "fake_type")
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.uri_offer,
                    json={
                        'offers': [self.fake_offer, fake_offer_1]
                    },
                ),
            ]
        )
    ...
    def test_list_leases(self, mock_ged):
        fake_lease_1 = fakes.make_fake_lease("fake_lease_id_1",
                                             "fake_lease_id_1",
                                             "fake_type",
                                             "fake_offer_uuid_1")
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.uri_lease,
                    json={
                        'leases': [self.fake_lease, fake_lease_1]
                    },
                ),
            ]
        )
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.uri_node,
                    json={
                        'nodes': [self.fake_node, self.fake_node_1]
                    },
                ),
            ]
        )
        ...
        self.register_uris(
            [
                dict(
                    ...
                    uri=self.uri_event,
                    json={
                        'events': [self.fake_event, self.fake_event_1]
                    },
                ),
            ]
        )
