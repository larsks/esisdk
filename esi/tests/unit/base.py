...
import esi.config as occ
import esi.connection
from esi.fixture import connection as os_fixture
from esi.tests import base
...
class TestCase(base.TestCase):
    ...
    def setUp(self, cloud_config_fixture='clouds.yaml'):
        ...
        self.fixtures_directory = 'esi/tests/unit/fixtures'
        self.os_fixture = self.useFixture(
            os_fixture.ConnectionFixture(project_id=fakes.PROJECT_ID)
        )

        # Isolate esi.config from test environment
        ...
        # self.useFixture(esi.cloud.RequestsMockFixture) and get all
        # the benefits.
    ...
    def _make_test_cloud(self, cloud_name='_test_cloud_', **kwargs):
        test_cloud = os.environ.get('ESISDK_OS_CLOUD', cloud_name)
        self.cloud_config = self.config.get_one(
            cloud=test_cloud, validate=True, **kwargs
        )
        self.cloud = esi.connection.Connection(
    ...
    def assertResourceEqual(self, actual, expected, resource_type):
        """Compares Resource against dictionary representing expected state.
    ...
    def assertResourceListEqual(self, actual, expected, resource_type):
        """Compares Resource lists against dictionary representing expected state.
